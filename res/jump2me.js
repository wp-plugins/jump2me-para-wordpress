...
jQuery(document).ready(function($){
	...
	$('#div_h3_twitter, #div_h3_wordpress, #div_h3_button, #div_h3_authentication, #div_h3_comment').css('display','none');
	
	$('#advanced_template').css('display','none');
	$('#toggle_advanced_template')
		.css('cursor','pointer')
		.append(' (clique para exibir)')
	...
	// Reset button
	$('#reset-jump2me,#unlink-jump2me').click(function(){
	...
	var jump2me = {
		...
		check: function( type ) {
			...
			if( type == 'path' ) {
				...
				post['username'] = $('#y_jump2me_login').val();
				post['password'] = $('#y_jump2me_passwd').val();
			}
			post['action'] = 'jump2me-check';
			post['_ajax_nonce'] = $('#_ajax_jump2me').val();
			post['jump2me_type'] = type;
			...
			$.ajax({
				url : ajaxurl,
				data : post,
				success : function(x) { jump2me.check_ok(x, '#check_'+type); },
				error : function(r) { jump2me.check_notok(r, '#check_'+type); }
			});
		},
